...
const SECTIONS: Record<string, (nav: Nav) => React.ReactNode> = {
  ...
  'why-now': () => <>
    ...
    <Grid cols={2}>
      {[
        ...
        { icon: '🤖', title: 'The ATS Black Hole', color: '#ef4444', body: 'Up to 75% of CVs are rejected by Applicant Tracking System bots before a human ever reads them. Candidates are invisible before the conversation even begins. InterviewMe bypasses the bot entirely — a recruiter watches a real person before any algorithm gets involved.' },
        { icon: '📄', title: 'The CV Is Broken', color: '#f59e0b', body: 'The CV is 50-year-old technology. It is a list of claims with zero proof. Anyone can write anything. InterviewMe replaces the claim with the evidence — a scored, AI-verified interview that shows exactly what the candidate can do. The market is ready for something that proves rather than asserts.' },
        { icon: '📹', title: 'Video Is Now Normal', color: A, body: 'COVID normalised video calls for everyone. Candidates are comfortable on camera in a way they never were before 2020. The psychological barrier to recording a video interview has collapsed — and the infrastructure (broadband, smartphones, webcams) is universal. The timing has never been better.' },
        { icon: '💸', title: 'Agency Fee Backlash', color: A2, body: 'Companies are aggressively cutting recruitment agency fees — 15–25% of first-year salary per hire. Finance teams are demanding in-house sourcing. InterviewMe gives internal recruiters a searchable pool of pre-verified, AI-scored candidates with no agency involved and no per-placement fee. The savings on a single senior hire cover a full year\'s subscription.' },
      ].map(f => (
  ...
  'patent': () => <>
    ...
    <div style={{ background: 'rgba(37,99,235,0.04)', border: '1px solid rgba(37,99,235,0.15)', borderRadius: 12, padding: '16px 20px', marginBottom: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {['🔍 Search by role, score, personality', '▶ Watch AI interview in full', '👍 Like — notifies candidate, drives sharing', '✉ Direct inbox — ATS bypassed'].map(s => <Tag key={s} color='#2563EB'>{s}</Tag>)}
      </div>
    </div>
    ...
    <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', color: A, marginBottom: 16 }}>The 16 Patentable Components</div>
    <Grid cols={2}>
      {[
        ...
        { n: '16', title: 'Personalised Career Avatar Guide', body: 'A system that generates and delivers a real-time, voice-rendered, personalised career briefing for any of 3,000+ global occupations. On demand, a synthetic AI interviewer — aware of the candidate\'s name, qualifications, hobbies, lifestyle preferences, and prior sessions — delivers a spoken walkthrough of a target career: its real-world highs and lows, salary range localised to the candidate\'s country, demand outlook, and lifestyle fit. The system then asks a conversational personalisation question, uses the response to infer career affinity, and surfaces 2-3 related roles the candidate would likely excel in. No existing system combines occupation-level intelligence from multiple global data sources (ESCO, O*NET, BLS, ONS), AI-generated narrative enrichment, voice synthesis, and candidate-profile personalisation into a single, real-time career guidance interaction.' },
      ].map(c => (
        <Card key={c.n}>
    ...
    <Grid cols={1}>
      <Card>
        {[
          ...
          { n: 'Claim 13', title: 'Personalised Career Avatar Guide', body: 'A system and method for delivering a real-time, voice-rendered, personalised career guidance experience comprising: receiving a candidate request for career information relating to a specific occupation; retrieving a structured occupation document from a multi-source intelligence corpus enriched from global occupational classification systems, labour market data APIs, and AI-generated narrative fields; retrieving a candidate profile comprising name, qualifications, interests, lifestyle preferences, country of residence, and prior session history; generating a personalised spoken script that incorporates occupation-specific intelligence — including real-world lifestyle, compensation localised to the candidate\'s country, demand outlook, and career progression narrative — woven with candidate-specific contextual references; synthesising the script into a voice audio stream via a neural text-to-speech engine rendered through a synthetic AI interviewer persona; delivering a conversational follow-up prompt to the candidate to elicit stated preferences and affinities; using the candidate response to rank and surface related occupations most aligned to stated and inferred preferences; and updating the candidate profile with expressed career interests for downstream personalisation. No known system combines multi-source global occupational intelligence, AI narrative enrichment, voice synthesis, candidate profile personalisation, and real-time affinity-based career recommendation into a single interactive guidance session.' },
          { n: 'Claim 14', title: 'Verifiable AI-Generated Interview Credential', body: 'A system and method for generating a tamper-evident, independently verifiable interview credential comprising: administering an interview session using an AI-generated, role-specific question set derived from a job specification and candidate profile such that questions are non-candidate-selectable; capturing candidate responses in real time via transcription and optional media recording; generating an AI-scored evaluation across multiple performance dimensions for each response; producing a session record comprising the AI-generated question set, response transcripts, per-question scores, aggregate performance score, coaching messages, session timestamp, and a cryptographic session identifier; sealing the session record against post-session modification; and making the sealed record available for verification by third parties including employers, recruiters, and professional networks as independently generated evidence of interview performance. The combination of AI-generated (non-candidate-selectable) questions and real-time response capture is the basis of independent verifiability — no equivalent system exists in which the question set is generated rather than selected and the resulting session record constitutes a portable credential.' },
          { n: 'Claim 15', title: 'Interview Portfolio and Best-Session Curation', body: 'A system and method for constructing, managing, and sharing a longitudinal interview performance portfolio comprising: aggregating a plurality of verifiable interview session records from a candidate over time across multiple roles, employers, and interview types; enabling the candidate to designate one or more session records as featured within a curated Interview Portfolio; computing a portfolio-level performance trajectory from aggregated per-session scores across multiple competency dimensions; generating a unified shareable portfolio credential comprising featured session records, trajectory visualisation, and aggregate competency profile; and providing a persistent, candidate-controlled access link through which authorised third parties — including employers, recruiters, and professional network contacts — may view the Interview Portfolio as a structured, time-stamped record of interview performance and demonstrated improvement. No known system enables a candidate to accumulate, curate, and share a longitudinal portfolio of AI-scored interview sessions as a verifiable professional credential.' },
          { n: 'Claim 16', title: 'Embeddable Interview Credential for CV and Professional Profile Distribution', body: 'A system and method for embedding a verifiable interview credential within candidate-owned professional documents and profiles comprising: generating a persistent, publicly accessible credential URL that resolves to a verified interview session record or Interview Portfolio for a named candidate; enabling the candidate to insert the credential URL within a curriculum vitae, professional profile, digital portfolio, or job application as a clickable reference; rendering, upon access by an authorised viewer, a structured display of the candidate\'s verified interview performance comprising AI-generated questions, session scores, competency ratings, and coaching messages; tracking employer and recruiter access events against the credential URL and reporting access notifications to the candidate; and providing an employer-facing interface allowing direct comparison of credential evidence from multiple candidates presented with the same role specification. The embeddable interview credential URL constitutes the first mechanism by which a candidate can include independently verifiable interview performance evidence within a CV — a capability with no known prior art.' },
        ].map(c => (
    ...
    <Card style={{ marginBottom: 28 }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {[
          'Fuses job advert + CV data', 'Generates personalised readiness packs', 'Multi-portal readiness visibility', 'Integrates employer feedback',
          'Triggers preparation from job listings', 'Magic Button — preparation at point of job discovery', 'Triggers preparation from recruiter actions',
          'Branded tokenised email distribution pipeline', 'Real-time multi-portal engagement signals', 'Structured readiness scoring',
          'Interview simulation with scoring overlays', 'Career pathway generation', 'Interview readiness infrastructure',
          'Multi-stage interview intelligence', 'Previous round context capture', 'Stage-escalating question prediction', 'Proprietary multi-stage question dataset',
          'Personalised voice-rendered career guidance', 'Candidate-profile-aware occupation briefing', 'Real-time career affinity recommendation',
          '🔒 AI-generated verifiable session record', '🔒 Tamper-evident interview credential', '🔒 Longitudinal Interview Portfolio',
          '🔒 Best-session curation & sharing', '🔒 Embeddable credential in CV', '🔒 Employer credential comparison view',
        ].map(n => <Tag key={n} color={n.startsWith('🔒') ? '#22c55e' : A}>{n}</Tag>)}
      </div>
      <div style={{ marginTop: 18, padding: '14px 16px', background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 10 }}>
        <div style={{ fontSize: 12, fontWeight: 800, color: '#22c55e', marginBottom: 6 }}>The Credential Claim — Most Commercially Significant</div>
        <div style={{ fontSize: 13, color: '#9090b0', lineHeight: 1.75 }}>
          Claims 14–16 cover the "interview as credential" concept: the first system in which an AI-generated interview session produces a verifiable, portable, embeddable proof of performance that a candidate owns and distributes — analogous in network effect to the LinkedIn connection. An Interview Portfolio link on a CV makes every subsequent employer aware of the platform through organic use. No prior art exists for this claim family.
        </div>
      </div>
    </Card>
