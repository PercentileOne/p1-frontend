...
import { aiProxyProtectionSettingsApi, type AiProxyProtectionSetting, type ApiError } from '../api/aiProxyProtectionSettingsApi'

export default function AiProtection() {
  const { token } = useAuth()
  const [caps, setCaps] = useState<AiProxyProtectionSetting | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      setCaps(await aiProxyProtectionSettingsApi.get(token))
    } catch (err) {
      setError((err as ApiError).error ?? 'Failed to load the AI protection setting.')
    } finally {
      setLoading(false)
    }
  }, [token])
  ...
  async function toggleCaps() {
    ...
    setError('')
    try {
      setCaps(await aiProxyProtectionSettingsApi.update(token, !caps.protectionEnabled))
    } catch (err) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.01em' }}>AI Protection</h1>
          <p style={{ fontSize: 13, color: 'var(--text-3)', marginTop: 4, maxWidth: 640 }}>
            Daily ceiling on the shared AI endpoint every AI feature uses (Learn, interviews, coaching, CV analysis). It has no login requirement, so this stops a script from running it up — real users never notice it.
          </p>
        </div>
        <button
          onClick={load}
        ...
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, padding: '20px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <ShieldCheck size={16} color={caps.protectionEnabled ? '#34D399' : 'var(--text-3)'} />
              <div>
                <p style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>Daily ceiling on the AI endpoint</p>
                <p style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2, maxWidth: 480 }}>
                  {caps.protectionEnabled
                    ? 'On (recommended) — per-visitor and site-wide daily limits apply. Turn off only as an emergency valve if it ever wrongly blocks real users.'
                    : 'Off — the AI endpoint is completely unmetered. Turn this back on as soon as you can.'}
                </p>
              </div>
            </div>
            <InlineToggle checked={caps.protectionEnabled} onChange={toggleCaps} disabled={saving} />
          </div>
          {caps.updatedBy && (
