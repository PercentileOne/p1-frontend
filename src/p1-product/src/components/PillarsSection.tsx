...
export default function PillarsSection() {
  ...
  return (
    <section id="pillars" style={{ background: 'var(--bg-2)', padding: isMobile ? '64px 0' : '96px 0' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: isMobile ? '0 20px' : '0 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: isMobile ? 12 : 18 }}>
          {pillars.map(p => (
            <div key={p.title} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: isMobile ? 18 : 26 }}>
              ...
              <h3 style={{ fontSize: isMobile ? 16 : 19, fontWeight: 800, color: 'var(--text)', letterSpacing: -0.5, marginBottom: 9 }}>{p.title}</h3>
