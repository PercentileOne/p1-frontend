  ...
  return (
    <div>
      ...
      <button
        ...
        style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', color: 'var(--blue)', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', padding: '4px 0', marginBottom: 16 }}
      >
        Advanced filters
          ...
          <div style={{ background: '#0c1220', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
            ...
            {results.map((c, i) => (
              <motion.div
                ...
                whileHover={{ background: 'rgba(79,142,247,0.08)' }}
                transition={{ duration: 0.15 }}
              >
                <div style={{ flex: '2 1 220px', minWidth: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 34, height: 34, borderRadius: '50%', overflow: 'hidden', background: 'linear-gradient(135deg,#4F8EF7,#7C3AED)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: '#fff', flexShrink: 0 }}>
                ...
                <div style={{ flex: '1 1 150px', minWidth: 0, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {c.remotePreference && <span style={{ fontSize: 10, fontWeight: 600, color: '#60A5FA', background: 'rgba(79,142,247,0.12)', padding: '2px 7px', borderRadius: 20, textTransform: 'capitalize' }}>{c.remotePreference}</span>}
                  {c.employmentTypePreference && <span style={{ fontSize: 10, fontWeight: 600, color: '#60A5FA', background: 'rgba(79,142,247,0.12)', padding: '2px 7px', borderRadius: 20, textTransform: 'capitalize' }}>{c.employmentTypePreference}</span>}
                </div>
                <div style={{ flex: '2 1 180px', minWidth: 0, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {c.interests.slice(0, 3).map(i => (
                    <span key={i} style={{ fontSize: 10, fontWeight: 600, color: '#A78BFA', background: 'rgba(167,139,250,0.12)', padding: '2px 8px', borderRadius: 20 }}>{i}</span>
                  ))}
