// Careers Explorer API client — copied from src/frontend/src/api/careersApi.ts (candidate
// portal) so the Alerts "Role" field can offer the same real job-title type-ahead, instead
// of free text only. Self-contained (own PROXY_BASE, no other app deps), so this is a plain
// copy rather than a shared package — see CLAUDE.md's "copy then trim" convention.
