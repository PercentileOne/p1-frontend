...
// Copy-trimmed from ShareModal.tsx rather than prop-extending it — that component's hero is
// hardcoded to a 0-100 score% display and "Explain · Interview Score" label baked into JSX, not
// parameterized for pass/fail. Platform list, getUrl builders, and copy-link logic are identical.
const PLATFORMS = [
  {
    id: 'linkedin',
    label: 'LinkedIn',
  ...
  },
  {
    ...
    getUrl: (url: string, text: string) =>
      `mailto:?subject=${encodeURIComponent('My Certification Exam Result')}&body=${encodeURIComponent(`${text}\n\n${url}`)}`,
  },
];

export function CertShareModal({ certName, passed, scaledScore, maxScore, shareUrl, onClose }: Props) {
  const [copied, setCopied] = useState(false);

  const shareText = passed
    ? `I just passed my ${certName} mock exam with a score of ${scaledScore}/${maxScore} on TheInterviewChair.com! 🎉`
    : `I just took a ${certName} mock exam on TheInterviewChair.com — scored ${scaledScore}/${maxScore}. Back to studying!`;
    ...
    <AnimatePresence>
      ...
      >
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '20px', overflow: 'hidden', width: '100%', maxWidth: '440px', pointerEvents: 'auto' }}>
          ...
          {/* Pass/fail hero */}
          <div style={{
            background: passed ? 'linear-gradient(135deg, #06281c 0%, #0f1729 100%)' : 'linear-gradient(135deg, #1a1040 0%, #0f1729 100%)',
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px' }}>
              <div>
                ...
                <div style={{ fontSize: '20px', fontWeight: 900, color: '#fff', lineHeight: 1.3 }}>
                  {certName}
                </div>
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '12px' }}>
              <div style={{ fontSize: '48px', fontWeight: 900, lineHeight: 1, color: passed ? '#34D399' : '#EF4444', fontVariantNumeric: 'tabular-nums' }}>
              ...
              <div style={{ marginBottom: '6px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                ...
                <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)' }}>Scaled score · Mock exam</div>
              </div>
            </div>
          </div>
          ...
          <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <div style={{ fontSize: '10px', color: 'var(--text-3)', textAlign: 'center', marginTop: '2px' }}>
              Recipients can view your result and question breakdown
            </div>
          </div>
        </div>
      </motion.div>
