...
function WaveformBars({ active, color, analyserNode }: {
  ...
  useEffect(() => {
    ...
    if (!active) { setHeights(BAR_SHAPES.map(() => 0.12)); return; }
    ...
    <div style={{ display: 'flex', gap: '3px', alignItems: 'center', height: '40px' }}>
      {heights.map((h, i) => (
        <motion.div
          ...
          style={{ width: '4px', height: '100%', background: active ? color : 'rgba(255,255,255,0.10)', borderRadius: '2px', transformOrigin: 'center' }}
        />
      ))}
    </div>
  );
...
export function InterviewerAvatar({ role, state, active, videoUrl, specialistTitle, analyserNode, onVideoEnded }: Props) {
  ...
  return (
    <div style={{
      flex: 1, background: profile.bg, borderRadius: '16px',
      border: `1px solid ${active ? (role === 'hr' ? 'rgba(167,139,250,0.4)' : 'rgba(79,142,247,0.4)') : 'rgba(255,255,255,0.06)'}`,
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      position: 'relative', overflow: 'hidden', transition: 'border-color 0.1s', padding: 0, minHeight: '300px',
    }}>
      <AnimatePresence>
        {active && (
      ...
      <div style={{ position: 'absolute', inset: 0 }}>
        ...
        <AnimatePresence>
          {videoUrl && (
            <motion.video key={videoUrl} src={videoUrl} autoPlay playsInline muted onEnded={onVideoEnded}
        ...
        <motion.div
          ...
          transition={active && state === 'speaking' ? { repeat: Infinity, duration: 2.2, ease: 'easeInOut' } : { duration: 0.4, ease: 'easeOut' }}
