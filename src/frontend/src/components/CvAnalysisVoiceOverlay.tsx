...
interface Props {
  ...
  title: string; // e.g. "Your CV" or "This Candidate's CV" — recruiter framing swaps this
  onClose: () => void;
}

const ACCENT = '#34D399';
const GLOW = 'rgba(52,211,153,0.28)';

// Copy-trimmed from CareerGuideOverlay.tsx — same glow/avatar treatment, plain TTS (no
// LiveAvatar) per Francis's own steer for this feature: cost-conscious, and this is the
// pattern already proven for "AI voice explains something to you" elsewhere in the product.
// narrativeScript is genuinely AI-written per-CV (unlike buildCareerScript's data-only
// template), so it's split into sentences here rather than pre-authored as an array of lines.
export function CvAnalysisVoiceOverlay({ narrativeScript, title, onClose }: Props) {
  const lines = useRef(
    narrativeScript.split(/(?<=[.!?])\s+/).map(s => s.trim()).filter(Boolean)
  ).current;
  ...
  useEffect(() => {
    ...
    cancelRef.current = speak(narrativeScript, 'hr', () => setSpeaking(false));
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '28px', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'default', userSelect: 'none' }}>
              <div style={{ fontSize: '13px', fontWeight: 700, color: ACCENT }}>Amina on {title}</div>
              {speaking && (
