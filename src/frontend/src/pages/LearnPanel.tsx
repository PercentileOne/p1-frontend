...
- Interview questions must be realistic interview questions a hiring manager would ask about this topic`;
  ...
  // Strategy: try server-side proxy, fall back to direct OpenAI call (key already in Vite bundle)
  ...
  // 1. Server-side proxy (works in production SWA)
  const via_proxy = await tryFetch('/api/ai-proxy', {}).catch(() => null);
  if (via_proxy) return via_proxy;

  // 2. Direct OpenAI (dev fallback — key is already in Vite bundle)
