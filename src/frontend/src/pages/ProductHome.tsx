...
/* ─────────────────────────────────────────────────────────────────────────────
   ProductHome — Explain.global cinematic PRODUCT PAGE
   Deployed at: product.explain.global  →  route "/"
   NOT the main platform (explain.global). This page showcases what's coming.
───────────────────────────────────────────────────────────────────────────── */
...
export default function ProductHome() {
  ...
  function handleWaitlist(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!input.value.includes("@")) { input.style.borderColor="rgba(239,68,68,.5)"; setTimeout(()=>(input.style.borderColor=""),1200); return; }
    btn.textContent="✓ You're on the list!"; btn.style.background="#34D399"; btn.style.boxShadow="0 8px 32px rgba(52,211,153,.35)"; btn.disabled=true;
  ...
  const css = `
    ...
    .ph-nav-links a:hover{color:#4F8EF7}
    .ph-nav-cta{padding:9px 20px;border-radius:8px;background:#4F8EF7;color:#fff;font-size:13px;font-weight:700;text-decoration:none;box-shadow:0 0 20px rgba(79,142,247,.3);transition:all .2s}
    .ph-nav-cta:hover{background:#2D5BFF;box-shadow:0 0 30px rgba(79,142,247,.5)}
    ...
    .ph-ir-preview{position:relative;border-radius:20px;overflow:hidden;border:1px solid rgba(79,142,247,.2);box-shadow:0 0 60px rgba(79,142,247,.12),0 40px 80px rgba(0,0,0,.5)}
    ...
    .ph-irm-mic{width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,#4F8EF7,#2563eb);border:none;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 0 14px rgba(79,142,247,0.35)}
    ...
    <>
      ...
      <section id="ph-learn" className="ph-feat">
        <div className="ph-c">
          <div className="ph-feat-head">
            ...
            <div className="ph-r" data-ph="" data-d="3" style={{marginTop:20}}><span className="ph-badge-gold">Launching with Explain.global</span></div>
          </div>
      ...
      <footer className="ph-footer">
        <div className="ph-footer-inner">
          ...
          <div style={{fontSize:11,color:"rgba(240,244,255,.35)"}}>© 2026 Percentile.One · product.explain.global · All rights reserved</div>
        </div>
      </footer>
    </>
  );
