...
const TOKEN_KEY = 'explain_employer_token';
const EMPLOYER_PERMISSION = 'CAN_VIEW_EMPLOYER_PORTAL';
...
interface AuthContextValue {
  ...
  token:      string | null;
  isLoading:  boolean;
  /** Validates a token against the shared backend before trusting it. Throws if invalid or not an employer account. */
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    authApi.getSession(stored)
      .then(session => {
        if (!session.permissions.includes(EMPLOYER_PERMISSION)) {
  ...
  const signIn = useCallback(async (newToken: string) => {
    ...
    if (!session.permissions.includes(EMPLOYER_PERMISSION)) {
      throw new Error("This account doesn't have employer access.");
    }
