          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 14 }}>
            {results.map(c => (
              <motion.div
                ...
                whileHover={{ y: -4, borderColor: 'rgba(52,211,153,0.5)', background: '#0f1a14', boxShadow: '0 8px 32px rgba(52,211,153,0.15)' }}
                transition={{ duration: 0.2, ease: 'easeOut' }}
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  <div style={{ width: 42, height: 42, borderRadius: '50%', overflow: 'hidden', background: 'linear-gradient(135deg,#34D399,#059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, color: '#fff', flexShrink: 0 }}>
                ...
                {c.interests.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 12 }}>
                    {c.interests.slice(0, 4).map(i => (
                      <span key={i} style={{ fontSize: 10, fontWeight: 600, color: '#34D399', background: 'rgba(52,211,153,0.12)', padding: '3px 8px', borderRadius: 20 }}>{i}</span>
                    ))}
                  </div>
                )}
              </motion.div>
